...
& $python @common `
    --model-config "configs\models\temporal_fixed_k1_v2.json" `
    --output "artifacts\phase1\temporal_k1_v2_screen1000.pt" `
    --metrics-output "artifacts\phase1\temporal_k1_v2_screen1000.metrics.jsonl"
if ($LASTEXITCODE -ne 0) {
    throw "K=1 screen failed with exit code $LASTEXITCODE"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "K=8 screen failed with exit code $LASTEXITCODE"
}
