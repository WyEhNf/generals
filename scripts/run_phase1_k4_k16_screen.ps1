...
foreach ($k in @(4, 16)) {
    & $python @common `
        --model-config "configs\models\temporal_fixed_k${k}_v2.json" `
        --output "artifacts\phase1\temporal_k${k}_v2_screen1000.pt" `
        --metrics-output "artifacts\phase1\temporal_k${k}_v2_screen1000.metrics.jsonl"
    if ($LASTEXITCODE -ne 0) {
        throw "K=$k screen failed with exit code $LASTEXITCODE"
    }
}
