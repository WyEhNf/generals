...
& $python scripts\train_bc.py `
    ...
    --sequence-length 64 `
    --burn-in 16 `
    --model-config configs\models\temporal_ewsc_explicit_k8_v2.json `
    --backbone-lr 1e-5 `
    --temporal-lr 1e-4 `
    --head-lr 5e-5 `
    ...
    --output artifacts\phase2\temporal_ewsc_explicit_k8_v2_screen1000.pt `
    --metrics-output artifacts\phase2\temporal_ewsc_explicit_k8_v2_screen1000.metrics.jsonl

if ($LASTEXITCODE -ne 0) {
    throw "Explicit EWSC K=8 screen failed with exit code $LASTEXITCODE"
}
