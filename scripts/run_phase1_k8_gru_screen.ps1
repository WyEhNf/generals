...
& $python scripts\train_bc.py `
    ...
    --sequence-length 64 `
    --burn-in 16 `
    --model-config configs\models\temporal_fixed_k8_gru_v2.json `
    --backbone-lr 1e-5 `
    --temporal-lr 1e-4 `
    --head-lr 5e-5 `
    ...
    --output artifacts\phase1\temporal_k8_gru_v2_screen1000.pt `
    --metrics-output artifacts\phase1\temporal_k8_gru_v2_screen1000.metrics.jsonl

if ($LASTEXITCODE -ne 0) {
    throw "K=8 GRU screen failed with exit code $LASTEXITCODE"
}
